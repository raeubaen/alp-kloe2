...
void ALP::Loop()
{
  ...
  TH1F *MpiNOPIETA = new TH1F("MpiNOPIETA","MpiNOPIETA",2040,0.,1020);
  TH1F *MpiNOPIETA_thecut = new TH1F("MpiNOPIETA_thecut","MpiNOPIETA_thecut",1040,0.,1020);
  TH1F *ErecNOPIETA = new TH1F("ErecNOPIETA","ErecNOPIETA",1040,0.,510.);
  TH1F *CostheNOPIETA = new TH1F("CostheNOPIETA","CostheNOPIETA",1000,-1.,1.);

  TH1F *MpiPIETA = new TH1F("MpiPIETA","MpiPIETA",2040,0.,1020);
  TH1F *MpiPIETA_thecut = new TH1F("MpiPIETA_thecut","MpiPIETA_thecut",2040,0.,1020);
  TH1F *ErecPIETA = new TH1F("ErecPIETA","ErecPIETA",2040,0.,510.);
  TH1F *CosthePIETA = new TH1F("CosthePIETA","CosthePIETA",1000,-1.,1.);
   
  TH1F *CostheMass = new TH1F("CostheMass","CostheMass",1000,-1.,1.);
  TH1F *CostheNomass = new TH1F("CostheNomass","CostheNomass",1000,-1.,1.);
  
  TH1F *Erecoil = new TH1F("Erecoil","Erecoil",4000,0.,1000.);
  TH1F *Erecoil_res= new TH1F("Erecoil_res","Erecoil_res",800,-200.,200.);
  TH2F *Erecmpi = new TH2F("Erecmpi","Erecmpi",1000,0.,1000.,2000,0.,1000.);
  
  TH2F *pippo = new TH2F("fit_vs_cluster_all","fit_vs_cluster_all",1600,0.,800.,1600,0.,800.);
  TH2F *pippo2= new TH2F("fit_vs_cluster_all_pieta","fit_vs_cluster_pieta",1600,0.,800.,1600,0.,800.);

  TH1F *enebha = new TH1F("enebha","enebha",4000,0.,500.);
  TH1F *eneeca = new TH1F("eneeca","eneeca",4000,0.,500.);
  ...
  //for (Long64_t jentry=0; jentry<50000;jentry++) {
    ...
    if (jentry%1000 == 0) cout << jentry/float(nentries)*100 << "%" << endl;
    ...
    for(i = 0; i < ngammas; i++){
      ...
      cluster_pos.SetXYZ(fit_par[nvars*i + 1],
			 fit_par[nvars*i+2],
			 fit_par[nvars*i+3]);
      TVector3 vertex_pos(&(fit_par[nvars*i + 15]));

      Rcl = TMath::Sqrt(TMath::Power(cluster_pos.X()-vertex_pos.X(), 2) +
			TMath::Power(cluster_pos.Y()-vertex_pos.Y(), 2) +
			TMath::Power(cluster_pos.Z()-vertex_pos.Z(), 2));
      Rcl_xy = TMath::Sqrt(TMath::Power(cluster_pos.X()-vertex_pos.X(), 2) +
			   TMath::Power(cluster_pos.Y()-vertex_pos.Y(), 2));
      ...
      gPx[i] = fit_par[nvars*i+0]*(cluster_pos.X()-vertex_pos.X())/(Rcl);
      gPy[i] = fit_par[nvars*i+0]*(cluster_pos.Y()-vertex_pos.Y())/(Rcl);
      gPz[i] = fit_par[nvars*i+0]*(cluster_pos.Z()-vertex_pos.Z())/(Rcl);
  ...
  auto *of = new TFile("out_misc.root", "recreate");
  of->cd();
  MpiNOPIETA->Write();
  MpiNOPIETA_thecut->Write();
  ErecNOPIETA->Write();
  CostheNOPIETA->Write();

  MpiPIETA->Write();
  MpiPIETA_thecut->Write();
  ErecPIETA->Write();
  CosthePIETA->Write();
